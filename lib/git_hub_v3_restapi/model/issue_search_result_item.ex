# NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
# https://openapi-generator.tech
# Do not edit the class manually.

defmodule ExOctocat.Model.IssueSearchResultItem do
  @moduledoc """
  Issue Search Result Item
  """

  @derive [Poison.Encoder]
  defstruct [
    :url,
    :repository_url,
    :labels_url,
    :comments_url,
    :events_url,
    :html_url,
    :id,
    :node_id,
    :number,
    :title,
    :locked,
    :active_lock_reason,
    :assignees,
    :user,
    :labels,
    :state,
    :assignee,
    :milestone,
    :comments,
    :created_at,
    :updated_at,
    :closed_at,
    :text_matches,
    :pull_request,
    :body,
    :score,
    :author_association,
    :draft,
    :repository,
    :body_html,
    :body_text,
    :timeline_url,
    :performed_via_github_app
  ]

  @type t :: %__MODULE__{
          :url => String.t(),
          :repository_url => String.t(),
          :labels_url => String.t(),
          :comments_url => String.t(),
          :events_url => String.t(),
          :html_url => String.t(),
          :id => integer(),
          :node_id => String.t(),
          :number => integer(),
          :title => String.t(),
          :locked => boolean(),
          :active_lock_reason => String.t() | nil,
          :assignees => [ExOctocat.Model.SimpleUser.t()] | nil,
          :user => SimpleUser | nil,
          :labels => [ExOctocat.Model.IssueSearchResultItemLabels.t()],
          :state => String.t(),
          :assignee => SimpleUser | nil,
          :milestone => Milestone | nil,
          :comments => integer(),
          :created_at => DateTime.t(),
          :updated_at => DateTime.t(),
          :closed_at => DateTime.t() | nil,
          :text_matches => [map()] | nil,
          :pull_request => ExOctocat.Model.IssueSimplePullRequest.t() | nil,
          :body => String.t() | nil,
          :score => float(),
          :author_association => ExOctocat.Model.AuthorAssociation.t(),
          :draft => boolean() | nil,
          :repository => ExOctocat.Model.Repository.t() | nil,
          :body_html => String.t() | nil,
          :body_text => String.t() | nil,
          :timeline_url => String.t() | nil,
          :performed_via_github_app => Integration | nil
        }
end

defimpl Poison.Decoder, for: ExOctocat.Model.IssueSearchResultItem do
  import ExOctocat.Deserializer

  def decode(value, options) do
    value
    |> deserialize(:assignees, :list, ExOctocat.Model.SimpleUser, options)
    |> deserialize(:user, :struct, ExOctocat.Model.SimpleUser, options)
    |> deserialize(:labels, :list, ExOctocat.Model.IssueSearchResultItemLabels, options)
    |> deserialize(:assignee, :struct, ExOctocat.Model.SimpleUser, options)
    |> deserialize(:milestone, :struct, ExOctocat.Model.Milestone, options)
    |> deserialize(:pull_request, :struct, ExOctocat.Model.IssueSimplePullRequest, options)
    |> deserialize(:author_association, :struct, ExOctocat.Model.AuthorAssociation, options)
    |> deserialize(:repository, :struct, ExOctocat.Model.Repository, options)
    |> deserialize(:performed_via_github_app, :struct, ExOctocat.Model.Integration, options)
  end
end
